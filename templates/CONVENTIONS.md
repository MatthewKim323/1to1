...
Goal: pixel-and-motion 1:1 rebuild of {{URL}} in {{STACK}}. Tailwind may be installed; exact values go in inline style objects and scoped `<style>` blocks.
...
- Images: copy from `assets/images/` to `public/img/<originalName>` and reference `/img/<originalName>`.
...
9. Text content verbatim from the tree, including typographic apostrophes. No em dashes anywhere (code, comments, copy you write).
