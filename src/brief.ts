...
export function writeBrief(root: string) {
  ...
  const L: string[] = [];
  L.push(`# REBUILD: ${meta.url ?? name}`, '', `> Captured ${meta.capturedAt ?? '?'} by 1to1. Everything in this folder is measured from the live page. Build from it; never eyeball a number.`, '');
  ...
  L.push(`- \`modules/*.mjs\`: the site's own code. The biggest one is the page. \`assets/\`, \`assets/svg/\`: every image / font / sprite def.`);
  ...
  L.push(`1to1 heights http://localhost:3777/ ${meta.url ?? '<live url>'}            # page + section heights at all 4 widths`);
