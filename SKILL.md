...
description: |
  ...
  with a box-diff loop until page heights match the live site to the pixel at every breakpoint.
...
```
/goal <the text in templates/GOAL.md, with the URL and routes filled in>
```
...
1to1 heights http://localhost:3777/ <url>                          # side by side, all widths
...
### 7. Ship
`bunx tsc --noEmit`, `next build`, `1to1 console` on every route, final `1to1 verify` table for every route. Report honestly: what matches, what does not, what was not checked.
